...
public class FaceActionLive {
    ...
    public void initActionLiveModel(final Context context, final String gazeModel,
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                int statusClose = -1;
                byte[] gazeModelContent = FileUitls.getModelContent(context, gazeModel);
                byte[] mouthcloseModelContent = FileUitls.getModelContent(context, mouthcloseModel);
                if (gazeModelContent.length != 0 && mouthcloseModelContent.length != 0) {
                    statusClose = nativeActionLiveModelInit(instanceIndex,
                            gazeModelContent, mouthcloseModelContent);
                    if (statusClose == 0) {
