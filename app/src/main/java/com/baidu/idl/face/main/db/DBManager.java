...
public class DBManager {
    ...
    public boolean addUser(User user) {
        ...
        try {
            ...
            cv.put("ctime", user.getCreateTime());
            cv.put("update_time", System.currentTimeMillis());
    ...
    public String queryLatestCTime() {
        Cursor cursor = null;
        String ctime = "2010/01/01 00:00:00";
        try {
            if (mDBHelper == null) {
                return null;
            }
            SQLiteDatabase db = mDBHelper.getReadableDatabase();
            cursor = db.rawQuery("SELECT * FROM user ORDER BY ctime DESC LIMIT 1", null);
            while (cursor != null && cursor.getCount() > 0 && cursor.moveToNext()) {
    ...
    public List<User> queryUserByUserId(String userId) {
        Cursor cursor = null;
        List<User> users = new ArrayList<>();
        try {
            if (mDBHelper == null) {
